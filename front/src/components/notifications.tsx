...
import {
	Bell, CircleAlert, TriangleAlert, Info, Filter, CheckCheck, Trash2, Clock, SortAsc, SortDesc
} from "lucide-react";
...
export const NotificationsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	const [notifications, setNotifications] = useState<AppNotification[]>(() => {
		...
		return [
			...
			// ... autres notifications initiales
		];
	});
								...
								>
									<DropdownMenuItem className={`focus:bg-accent/50 px-3 py-3 border-b last:border-b-0 transition-colors ${!n.read ? "bg-blue-50/50" : ""}`}>
										<div className="w-full flex items-start gap-3 relative">
											...
											<div
												className={`mt-0.5 p-1.5 rounded-full ${n.severity === "alert" ? "bg-destructive/10" : n.severity === "warning" ? "bg-warning/10" : "bg-muted/10"
													}`}
											>
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2">
													...
													<span
														className={`text-[10px] px-1.5 py-0.5 rounded-full border ${n.severity === "alert"
