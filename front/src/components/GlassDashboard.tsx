...
// Composants internes du dashboard
...
// Icônes Lucide React
import { Activity, BarChart, Battery, Bell, Gauge, Leaf, Minus, Settings, ShieldCheck,
TrendingDown, TrendingUp, Wifi, X, MessageCircle, SunIcon, RotateCcw, Eye,
TreePine, Recycle, Wind, Droplets, FileText, Thermometer, Cloud, AlertTriangle,
Volume2, Scale } from "lucide-react";

// Composants de graphiques Recharts
import { AreaChart, LineChart, Area, CartesianGrid, Legend, Line, ResponsiveContainer,
...
/**
 * Composant Glass - Applique un effet de verre avec transparence et flou
 * @param children - Contenu à afficher dans le conteneur glass
...
export function GlassDashboard() {
  ...
  // États pour la sélection des périodes et métriques
  ...
  // État pour les seuils configurables
  ...
  /**
   * Données simulées pour chaque type de capteur du projet
   * Ces données représentent les mesures collectées sur une période donnée
   */
  
  // Capteur DHT22 - Température et humidité (°C)
  const dht22Data = [
    { value: 18 }, { value: 22 }, { value: 19 }, { value: 25 }, { value: 21 },
    { value: 23 }, { value: 20 }, { value: 24 }, { value: 21 }
  ];

  // Capteur MQ-135 - Qualité de l'air/gaz polluants (ppm)
  const mq135Data = [
    { value: 320 }, { value: 340 }, { value: 360 }, { value: 380 }, { value: 400 },
    { value: 420 }, { value: 440 }, { value: 460 }, { value: 480 }
  ];

  // Capteur 801S - Niveau de vibration (Hz)
  const s801VibrationData = [
    { value: 45 }, { value: 50 }, { value: 48 }, { value: 55 }, { value: 52 },
    { value: 58 }, { value: 56 }, { value: 60 }, { value: 56 }
  ];

  // Capteur YF-S401 - Débit et consommation d'eau (L/min)
  const yfS401Data = [
    { value: 100 }, { value: 120 }, { value: 110 }, { value: 130 }, { value: 125 },
    { value: 140 }, { value: 135 }, { value: 150 }, { value: 145 }
  ];

  // Capteur KY-038 - Niveau de bruit (dB)
  const ky038Data = [
    { value: 300 }, { value: 320 }, { value: 310 }, { value: 330 }, { value: 340 },
    { value: 350 }, { value: 360 }, { value: 370 }, { value: 380 }
  ];

  // Cellule de pesage - Balance pour ordures (Kg)
  const balanceData = [
    { value: 120 }, { value: 125 }, { value: 118 }, { value: 130 }, { value: 127 },
    { value: 132 }, { value: 129 }, { value: 135 }, { value: 131 }
  ];
  
  // Aliases pour capteurs utilisant des données similaires
  const mq9Data = mq135Data;        // MQ-9 réutilise les données MQ-135
  const gyBme280Data = dht22Data;   // GY-BME280 réutilise les données DHT22

  // ===== DONNÉES D'ACTIVITÉ MENSUELLE =====
  
  /**
   * Données d'activité par mois avec indicateurs de croissance
   */
  ...
  /**
   * Récupère la dernière valeur d'un tableau de données
   * @param arr - Tableau de données avec propriété 'v' optionnelle
   * @returns La dernière valeur ou undefined si le tableau est vide
   */
  ...
  /**
   * Retourne l'icône de tendance appropriée selon la valeur
   * @param value - Valeur numérique pour déterminer la tendance
   * @returns Composant icône React
   */
  ...
  /**
   * Retourne le titre de la période sélectionnée
   */
  ...
  /**
   * Retourne le titre de la métrique sélectionnée
   */
  ...
  // Dernières valeurs pour chaque type de capteur
  ...
  // Calculs pour la synthèse des capteurs
  ...
  // KPIs calculés à partir des données d'activité
  ...
  /**
   * Génère et télécharge un rapport PDF avec les recommandations environnementales
   * @param event - Événement de clic du bouton
   */
  function navigateToBilling(event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void {
    event.preventDefault();

    // Données des recommandations à inclure dans le PDF
    const recommendations = [
      { 
        title: "Réduire pollution sonore", 
        desc: "KY-038 détecte +15% bruit - Actions nécessaires", 
        priority: "URGENT", 
        impact: "Élevé" 
      },
      { 
        title: "Optimiser consommation eau", 
        desc: "YF-S401 : débit excessif -15% possible", 
        priority: "MOYEN", 
        impact: "Moyen" 
      },
      { 
        title: "Améliorer qualité air", 
        desc: "MQ-135 : taux polluants élevés - ventilation", 
        priority: "URGENT", 
        impact: "Élevé" 
      },
      { 
        title: "Réduire émissions CO₂", 
        desc: "Données capteurs suggèrent économie énergie", 
        priority: "MOYEN", 
        impact: "Élevé" 
      },
      { 
        title: "Zone de confort thermique", 
        desc: "DHT22 : optimiser chauffage -8°C possible", 
        priority: "FAIBLE", 
        impact: "Moyen" 
      },
      { 
        title: "Certification environnementale", 
        desc: "Données conformes ISO 14001 - validation", 
        priority: "FAIBLE", 
        impact: "Faible" 
      },
      { 
        title: "Surveillance vibrations", 
        desc: "801S : réduire nuisances mécaniques", 
        priority: "FAIBLE", 
        impact: "Faible" 
      }
    ];
    ...
    // Titre du tableau des recommandations
    doc.setFont("helvetica", "bold");
    doc.text("Recommandations :", 20, y);
    y += 8;
    doc.setFont("helvetica", "normal");

    // Génération des recommandations dans le PDF
    ...
    <div className={classNames("min-h-screen", bgGrad)}>
      ...
      <div className="relative z-10 p-3 space-y-4 overflow-y-auto max-h-screen scrollbar-hide">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-8 gap-3">
          ...
          {/* Capteur 801S - Niveau de Vibration */}
          <GlassMetricCard
          ...
          {/* Capteur YF-S401 - Débit & Consommation d'eau */}
          <GlassMetricCard
          ...
          {/* Capteur GY-BME280 - Pression Atmosphérique */}
          <GlassMetricCard
            ...
            icon={<Gauge className="w-3 h-3 text-white" />}
            small
          />

          {/* Capteur MQ-9 - Gaz toxiques */}
          <GlassMetricCard
          ...
          {/* Capteur KY-038 - Niveau de Bruit */}
          <GlassMetricCard
            ...
            icon={<Volume2 className="w-3 h-3 text-white" />}
            small
          />

          {/* Cellule de pesage - Balance ordures */}
          <GlassMetricCard
        ...
        {/* ===== LIGNE 2 : ALERTES, ÉTAT CAPTEURS ET RECOMMANDATIONS ===== */}
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="h-96 rounded-2xl border border-white/30 bg-white/40 backdrop-blur-xl shadow-xl p-4 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="px-2 py-1 rounded-full bg-blue-500/10 text-blue-600 text-xs font-medium border border-blue-200">
                {activesSensors}/{totalSensors}
              </span>
            </div>
            ...
            {/* Liste des capteurs avec leurs états */}
            <div className="flex-1 space-y-2 overflow-y-auto">
              {[
          ...
          {/* ===== SECTION RECOMMANDATIONS DÉMARCHE VERTE ===== */}
          <div className="h-96 rounded-2xl border border-white/30 bg-white/40 backdrop-blur-xl shadow-xl p-4 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto">
              {[
                { 
                  title: "Réduire pollution sonore", 
                  desc: "KY-038 détecte +15% bruit - Actions nécessaires", 
                  icon: <Wind className="w-3 h-3 text-blue-500" />, 
                  priority: "URGENT", 
                  color: "red", 
                  impact: "Élevé" 
                },
                { 
                  title: "Optimiser consommation eau", 
                  desc: "YF-S401 : débit excessif -15% possible", 
                  icon: <Droplets className="w-3 h-3 text-cyan-500" />, 
                  priority: "MOYEN", 
                  color: "blue", 
                  impact: "Moyen" 
                },
                ...
                { 
                  title: "Réduire émissions CO₂", 
                  desc: "Données capteurs suggèrent économie énergie", 
                  icon: <Recycle className="w-3 h-3 text-emerald-500" />, 
                  priority: "MOYEN", 
                  color: "green", 
                  impact: "Élevé" 
                },
                { 
                  title: "Zone de confort thermique", 
                  desc: "DHT22 : optimiser chauffage -8°C possible", 
                  icon: <SunIcon className="w-3 h-3 text-yellow-500" />, 
                  priority: "FAIBLE", 
                  color: "orange", 
                  impact: "Moyen" 
                },
                { 
                  title: "Certification environnementale", 
                  desc: "Données conformes ISO 14001 - validation", 
                  icon: <ShieldCheck className="w-3 h-3 text-indigo-500" />, 
                  priority: "FAIBLE", 
                  color: "blue", 
                  impact: "Faible" 
                },
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-white/30 bg-white/20 backdrop-blur-xl shadow-xl p-3">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3">
              ...
              {/* Sélecteurs de période et métrique */}
              ...
              <div className="grid md:grid-cols-5 gap-4">
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  <Glass className="p-3">
                    ...
                    {/* Message pour autres polluants */}
                    {(["pm25","pm10","no2","o3"] as SensorKey[]).includes(selectedMetric) && (
                      <div className="text-xs text-gray-600">
                        Réglez les seuils OMS/locaux dans la section "Polluants" ci-dessous.
                      </div>
                    )}
                  </Glass>
                </div>
        ...
        {/* ===== LIGNE 4 : COMPARATIF DES CAPTEURS DU PROJET ===== */}
        <div className="mt-6">
          <Glass className="p-4">
            <SectionTitle 
              icon={<Leaf className="w-5 h-5 text-emerald-600" />} 
              title="Capteurs du Projet — Comparatif" 
            />
            
            {/* Graphique comparatif multi-capteurs */}
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
        ...
        {/* ===== BOUTON FLOTTANT ASSISTANT CHAT ===== */}
        <button
        ...
        {/* Composant ChatAssistant externe */}
        <ChatAssistant />
      </div>
    </div>
  );
